...
| :---- | :---- | :---- | :---- | :---- |
| JSON-TEXT-1 | For texts shorter than 2^32 - 1 characters, all of them code points (at most U+10FFFF): `parse(t)` is `Some` exactly when `t` matches RFC 8259's `JSON-text` rule (§2 to §7) and holds no `\u` escape of an unpaired surrogate (§8.2) | Proved | proved | ezjson/LAWS.bend text_some; ezjson/LAWS.bend text_derives |
...
| JSON-TEXT-5 | For texts shorter than 2^32 - 1 characters: for every text `t`, `parse` of U+FEFF followed by `t` is `None` (§8.1) | Proved | proved | ezjson/LAWS.bend bom_none |
...
| JSON-NUM-2 | For texts shorter than 2^32 - 1 characters: for every `s` with `num.ok(s)`, `parse(s)` is `Some{JNum{s}}` and `print` of it is `s`: a number keeps its spelling | Proved | proved | ezjson/LAWS.bend parse_num; ezjson/LAWS.bend print_num |
...
| JSON-STR-4 | For texts shorter than 2^32 - 1 characters: two object names compare equal in `get` exactly when their decoded characters are equal, whatever escapes spelled them (§8.3) | Proved | proved | ezjson/LAWS.bend get_decoded |
