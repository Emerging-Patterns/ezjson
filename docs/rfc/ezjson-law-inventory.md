...
| Grammar, WP-G | done | JSON-NUM-2 (`parse_num`, `print_num`), JSON-TEXT-3 (`two_none`), JSON-TEXT-4 (`bare_word`) and JSON-TEXT-5 (`bom_none`) proved. JSON-STR-3 proved (`raw_kept`, `ctl_none`). JSON-STR-2 proved (`esc_decodes`, `esc_bad`, `esc_u`, `esc_pair`, `esc_u_bad`, `esc_lo_bad`, `esc_hi_bad`). JSON-STR-4 proved (`get_decoded`). JSON-TEXT-1 proved (`text_some`, `text_derives`). JSON-TEXT-2 proved (`ws_same`, `ins_none`). |
| Last, JSON-TREE-6 | done | JSON-TREE-6 proved (`as_u32_num`): `as_u32(num(s))`, read as a Nat, is `Some` of the value exactly when `s` is `decimal` (RFC 8259's int without the minus: `0`, or a digit 1-9 and digits) and the value `fits` in 32 bits, and `None` otherwise. The bound is stated by halving (`fits(32n, v)`) because the checker expands a closed Nat to unary, so 4294967295n or 2^32 cannot appear in a type; inside the proofs 2^32 is `pow2u(32n, uu)`, held back by a unit. The proof goes through base's `U32.read` as written: word addition (`w_adc`), shifts (`w_shl`, `w_sho`), multiplication (`w_mulgo`) and division by ten (`dm_e`, `dm_b`) as Nat arithmetic modulo 2^32, one step of the read (`step_ok`, `step_bad_d`, `step_bad_t`), then the read over digits (`read_all`) and over a text with a non-digit (`read_bad`). So JSON-TREE-6 no longer leans on JSON-TRUST-2's `U32.read` half. |
| Cursor, WP-C | in progress | JSON-PULL-1 proved (`pull_events`, `pull_ends`): the events `next` yields from `cursor(t)`, for a text shorter than 2^32 characters, reach the end exactly when `parse(t)` is `Some{j}`, and then they are `events(j)` followed by the end. One direction is an induction on the parse derivation (`gv_go`), the other walks the cursor in step with the character lexer and the parser (`frel`, `cminv`). The fuel is a budget of nested levels and no lemma normalizes it (REVIEW-P4). JSON-PULL-4 open. |
| Three to five | open | see the RFC's Rollout |

Laws now: 59, all quantified, 0 closed. `coverage` warnings: 93.
