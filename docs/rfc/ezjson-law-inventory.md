...
| Pretty, JSON-PRINT-4 | done | `pretty` added and JSON-PRINT-4 proved (`pretty_parse`): for a well-formed value, `parse(pretty(j))` is what `parse(print(j))` is. The proof builds a derivation of `pretty`'s text from the value and applies `text_some` (docs/rfc/ezjson-parse-proofs.md, Update). |
