...
| Grammar, WP-G | in progress | JSON-NUM-2 (`parse_num`, `print_num`), JSON-TEXT-3 (`two_none`), JSON-TEXT-4 (`bare_word`) and JSON-TEXT-5 (`bom_none`) proved. JSON-STR-3 proved (`raw_kept`, `ctl_none`). JSON-STR-2 proved (`esc_decodes`, `esc_bad`, `esc_u`, `esc_pair`, `esc_u_bad`, `esc_lo_bad`, `esc_hi_bad`). JSON-STR-4 proved (`get_decoded`). JSON-TEXT-1 proved (`text_some`, `text_derives`). Left: JSON-TEXT-2 |
