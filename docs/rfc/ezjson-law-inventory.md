...
| Parser, WP-P1 | done | `parse_tj`: the parser on a well-formed value's tokens gives the value back, strings and span keys owned and fixed (`canon`). JSON-PRINT-3 proved: `parse_wf`, every value `parse` returns is well-formed, for every text, as an invariant of the parser's state. Next: WP-H, parse over span tokens against their owned forms, then JSON-PRINT-1 |
